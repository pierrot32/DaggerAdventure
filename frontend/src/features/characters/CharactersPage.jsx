...
export default function CharactersPage() {
	...
	useEffect(() => {
		listCharacters()
			...
			.catch((error) =>
				setState({ loading: false, deletingId: "", error: error.message }),
			);
	}, []);
				...
				<div className={styles.grid}>
					{characters.length === 0 && (
						<div className={styles.empty}>
							...
							<p className="muted">
								Create a character and keep their choices together in your
								vault.
							</p>
							<Link to="/characters/create">
								<Button variant="text">Start character creation</Button>
							</Link>
						</div>
					)}
					{characters.map((character) => (
						<article className={styles.card} key={character.id}>
							...
							>
								...
								<p className="muted">
									{character.class_id} · {character.ancestry_id} ·{" "}
							...
							<div className={styles.cardActions}>
								<button
									...
									disabled={state.deletingId === character.id}
									onClick={(event) => handleDelete(event, character)}
								>
									{state.deletingId === character.id
										? "Deleting..."
										: "Delete character"}
								</button>
							</div>
