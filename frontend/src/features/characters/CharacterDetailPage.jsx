...
const nextTrackValue = (current, index) =>
	index + 1 === current ? index : index + 1;
...
const advancementOptions = [
	{
		id: "traits",
		title: "Increase two traits by +1",
		detail: "Choose two traits that are not marked in the current tier.",
	},
	{
		id: "hit_points",
		title: "Gain one Hit Point slot",
		detail: "Permanently increase your maximum Hit Points by 1.",
	},
	{
		id: "stress",
		title: "Gain one Stress slot",
		detail: "Permanently increase your maximum Stress by 1.",
	},
	{
		id: "experiences",
		title: "Increase two Experiences by +1",
		detail: "Choose two Experiences already on your sheet.",
	},
	{
		id: "domain_card",
		title: "Gain an additional domain card",
		detail: "Choose a card from an available level and lower.",
	},
	{
		id: "evasion",
		title: "Gain +1 Evasion",
		detail: "Permanently increase your Evasion by 1.",
	},
	{
		id: "subclass",
		minTier: 3,
		title: "Take an upgraded subclass card",
		detail: "Record the upgraded subclass choice.",
	},
	{
		id: "proficiency",
		minTier: 3,
		title: "Increase your Proficiency by +1",
		detail: "Permanently increase Proficiency by 1.",
	},
	{
		id: "multiclass",
		minTier: 3,
		title: "Take a multiclass option",
		detail:
			"Choose an additional class and cross out the other multiclass option.",
	},
];
...
function domainCardsFromBook(book) {
	return (book?.domains || []).flatMap((domain) =>
		Object.entries(domain)
			.filter(
				([key, cards]) => /^level_\d+_cards$/.test(key) && Array.isArray(cards),
			)
			.flatMap(([key, cards]) =>
				cards.map((card) => ({
					...card,
					level: Number(key.match(/\d+/)[0]),
					domain: domain.name,
					domainId: domain.id,
				})),
			),
...
function choiceSummary(choice, character, book) {
	if (choice?.id === "traits")
		return `Traits: ${(choice.values || []).map(titleize).join(", ")}`;
	if (choice?.id === "experiences")
		return `Experiences: ${(choice.values || []).map((index) => character.experiences?.[index]?.name || `Experience ${Number(index) + 1}`).join(", ")}`;
	if (choice?.id === "domain_card") {
		const card = domainCardsFromBook(book).find(
			(item) => item.id === choice.value && item.domainId === choice.domain_id,
		);
		return `Domain card: ${card?.name || choice.value || "selected card"}`;
	}
	if (choice?.id === "multiclass")
		return `Multiclass: ${titleize(choice.value)}`;
	return (
		advancementOptions.find((option) => option.id === choice?.id)?.title ||
		choice?.id
	);
}

const withoutGoldInventory = (inventory) =>
	(Array.isArray(inventory) ? inventory : []).filter(
		(item) => !/gold/i.test(typeof item === "string" ? item : item?.name || ""),
	);
const editableInventory = (inventory) =>
	withoutGoldInventory(inventory).map((item, index) =>
		typeof item === "string"
...
const editableCharacter = (value) => ({
	...
	look_description: value.look_description || "",
	experiences: Array.isArray(value.experiences)
		? value.experiences.map((item) =>
				typeof item === "string" ? { name: item, modifier: 2 } : { ...item },
			)
		: [],
	equipment: {
		...(value.equipment || {}),
	...
	background_story: value.background_story || "",
	background_notes: value.background_notes || "",
	birth_city: value.birth_city || "",
	family_members: Array.isArray(value.family_members)
		? value.family_members.map((member) => ({ ...member }))
		: [],
...
export default function CharacterDetailPage({ mode = "sheet" }) {
		...
		Promise.all([
			...
			.then(([nextCharacter, adventureResult, nextBook]) => {
				...
				setStats(
					normalizeStats(
						nextCharacter.stats,
						deriveSheet(nextCharacter, nextBook),
					),
				);
	...
	const setTrack = (key, next) =>
		persist({ ...stats, [key]: { ...stats[key], current: next } });
	const setGold = (key, next) =>
		persist({ ...stats, gold: { ...stats.gold, [key]: next } });
	...
	const updateEquipmentField = (field, value) =>
		setEditForm((current) => ({
			...current,
			equipment: { ...current.equipment, [field]: value },
		}));
	const updateExperience = (index, value) =>
		setEditForm((current) => ({
			...current,
			experiences: current.experiences.map((experience, experienceIndex) =>
				experienceIndex === index ? { ...experience, name: value } : experience,
			),
		}));
	const addExperience = () =>
		setEditForm((current) => ({
			...current,
			experiences: [...current.experiences, { name: "", modifier: 2 }],
		}));
	...
	const updateInventoryItem = (index, field, value) =>
		setEditForm((current) => ({
			...
			equipment: {
				...current.equipment,
				inventory: current.equipment.inventory.map((item, itemIndex) =>
		...
		setEditForm((current) => ({
			...
			equipment: {
				...
				inventory: [
					...
					{ id: `custom-${Date.now()}`, name: "", quantity: 1 },
				],
			},
		}));
	const removeInventoryItem = (index) =>
		setEditForm((current) => ({
			...
			equipment: {
				...current.equipment,
				inventory: current.equipment.inventory.filter(
	...
	const removeFamilyMember = (index) =>
		setEditForm((current) => ({
			...current,
			family_members: current.family_members.filter(
	...
	const saveCharacter = async () => {
		...
		try {
			const updated = await updateCharacter(characterId, {
				...editForm,
				experiences: editForm.experiences.filter((experience) =>
				...
				equipment: {
					...editForm.equipment,
					inventory: editableInventory(editForm.equipment.inventory),
				},
			});
			setCharacter(updated);
	...
	const heritage = [
		...
		character.secondary_ancestry_id,
		...
		return (
			<section className={styles.sheet}>
				<div className={styles.topBar}>
					...
					<Link
						to={`/characters/${characterId}/profile`}
				...
				<header className={styles.pageHeading}>
					<div>
						...
						<p className="muted">
							Update your character details, story, equipment, and portrait.
						</p>
					</div>
					<div className={styles.pageHeadingActions}>
						<Link
							to={`/characters/${characterId}/notes`}
						...
						<label className={styles.adventure}>
							...
							<select
								...
								onChange={updateAdventure}
								disabled={state.saving || adventureState.loading}
							>
								...
								{selectedAdventure &&
									!adventures.some(
										...
										<option value={selectedAdventure}>
											Current linked adventure
										</option>
									)}
				...
				{adventureState.error && (
					<p className={styles.error} role="status">
						Available adventures could not be loaded. The current link is
						preserved.
					</p>
				)}
	...
	return (
		<section className={styles.sheet}>
			...
			<div className={styles.editActions}>
				...
				{character.class_id === "druid" && (
					<Link
						to={`/characters/${characterId}/beastforms`}
						className={styles.cancelButton}
					>
						Beast forms
					</Link>
			...
			<header className={styles.nameplate}>
				<div className={styles.classBlock}>
					<h2>
						{classInfo?.name?.toUpperCase() || titleize(character.class_id)}
					</h2>
				...
				<div className={styles.identity}>
					...
					<Field
						label="Subclass"
						value={subclassInfo?.name || titleize(character.subclass_id)}
					/>
				</div>
				{isOwner ? (
					<button
			...
			<div className={styles.columns}>
				<div className={styles.left}>
					<div className={styles.defenses}>
						<Shield
							label="Evasion"
							value={derived.evasion}
							note="Class + level"
						/>
						<Shield
							label="Armor"
							value={derived.armorScore}
							note="Armor score"
						/>
								...
								onSelect={
									isOwner
										? (index) =>
												setTrack(
													"armor",
													nextTrackValue(stats.armor.current, index),
												)
										: undefined
								}
								label="Armor slots"
					...
					<Panel title="Damage &amp; Health">
						<p className={styles.hint}>
							Thresholds include your current level.
						</p>
						<div className={styles.thresholds}>
							...
							<Threshold
								name="Major damage"
								mark="Mark 2 HP"
								value={derived.thresholds.major}
							/>
							<Threshold
								name="Severe damage"
								mark="Mark 3 HP"
								value={derived.thresholds.severe}
							/>
						</div>
						<TrackRow
							label="HP"
							...
							onSelect={
								isOwner
									? (index) =>
											setTrack(
												"hit_points",
												nextTrackValue(stats.hit_points.current, index),
											)
									: undefined
							}
						/>
							...
							onSelect={
								isOwner
									? (index) =>
											setTrack(
												"stress",
												nextTrackValue(stats.stress.current, index),
											)
									: undefined
							}
						/>
					...
					<Panel title="Hope">
							...
							onSelect={
								isOwner
									? (index) =>
											setTrack(
												"hope",
												nextTrackValue(stats.hope.current, index),
											)
									: undefined
							}
							label="Hope"
						...
						{classInfo?.hope_feature && (
							<p className={styles.feature}>
								<strong>{classInfo.hope_feature.name}.</strong>{" "}
								{classInfo.hope_feature.text}
							</p>
						)}
					</Panel>
									...
									<b>
										{signed(
											(experience.modifier ?? 2) +
												(derived.experienceBonuses[index] || 0),
										)}
									</b>
								</li>
							))}
							{(character.experiences || []).length === 0 && (
								<li className="muted">No experiences recorded.</li>
							)}
						</ul>
					</Panel>
						...
						<div className={styles.gold}>
								...
								onSelect={
									isOwner
										? (index) =>
												setGold(
													"handfuls",
													nextTrackValue(stats.gold.handfuls, index),
												)
										: undefined
								}
							/>
							<GoldRow
								...
								onSelect={
									isOwner
										? (index) =>
												setGold("bags", nextTrackValue(stats.gold.bags, index))
										: undefined
								}
							/>
							<GoldRow
								...
								onSelect={
									isOwner
										? (index) =>
												setGold(
													"chest",
													nextTrackValue(stats.gold.chest, index),
												)
										: undefined
								}
							/>
					...
					{classInfo?.class_features?.length > 0 && (
						<Panel title="Class feature">
							{classInfo.class_features.map((feature) => (
								<p className={styles.feature} key={feature.name}>
				...
				<div className={styles.right}>
						...
						<div className={styles.traits}>
							{TRAIT_IDS.map((trait) => (
								<div className={styles.trait} key={trait}>
									...
									<strong>
										{signed(
											(Number(character.traits?.[trait]) || 0) +
												(derived.traitBonuses[trait] || 0),
										)}
									</strong>
					...
					<Panel title="Active weapons">
						...
						<WeaponRow
							title="Primary"
							weapon={primary}
							fallback={equipment.primary}
						/>
						<WeaponRow
							title="Secondary"
							weapon={secondary}
							fallback={equipment.secondary}
						/>
					</Panel>
					...
					<Panel title="Active armor">
						<div className={styles.statLine}>
							<div>
								<span>Name</span>
								<strong>
									{armor?.name || titleize(equipment.armor) || "—"}
								</strong>
							</div>
							<div>
								<span>Base thresholds</span>
								<strong>{armor?.thresholds || "—"}</strong>
							</div>
							<div>
								<span>Base score</span>
								<strong>{armor?.armor_score ?? "—"}</strong>
							</div>
						</div>
						{armor?.feature && (
							<p className={styles.feature}>{armor.feature}</p>
						)}
					</Panel>

					<Panel title="Inventory">
						<ul className={styles.inventory}>
							{[equipment.potion, ...withoutGoldInventory(equipment.inventory)]
								.filter(Boolean)
								.map((item, index) => (
									<li
										key={
					...
					<Panel title="Background">
						<div className={styles.statLine}>
							<div>
								...
								<strong>{character.birth_city || "—"}</strong>
							</div>
						</div>
						{character.background_story && (
							<p className={styles.feature}>{character.background_story}</p>
						)}
						{character.background_notes && (
							<p className={styles.hint}>{character.background_notes}</p>
						)}
					</Panel>
							...
							<div className={styles.cards}>
								{character.domain_cards.map((card) => (
									<article key={card.id || card.name}>
										<span>
											{card.domain} · {card.type} · recall {card.recall_cost}
										</span>
...
function AdvancementPanel({ character, book, onClose, onAdvanced }) {
	...
	const classDomains = new Set(
		book?.classes?.find((item) => item.id === character.class_id)?.domains ||
			[],
	);
	const ownedCardKeys = new Set(
		(character.domain_cards || []).map(
			(card) => `${card.domainId || card.domain_id || ""}:${card.id || ""}`,
		),
	);
	const availableCards = domainCardsFromBook(book)
		.filter((card) => classDomains.has(card.domainId))
	...
	const availableTraits = (() => {
		...
		(character.advancements || []).forEach((entry) => {
			...
			(entry.choices || [])
				.filter((choice) => choice.id === "traits")
				.flatMap((choice) => choice.values || [])
				.forEach((trait) => marked.add(trait));
		});
		if (nextLevel === 5 || nextLevel === 8) marked.clear();
		return TRAIT_IDS.filter((trait) => !marked.has(trait));
	})();
	const options = advancementOptions.filter(
		(option) => !option.minTier || option.minTier <= tier,
	);
	const previousTier = tier - 1;
	const previousTierMarked = new Set(
	...
	const previousOptions = options
		.filter((option) => !option.minTier || option.minTier <= previousTier)
	...
	const toggleOption = (id, sourceTier = tier) =>
		setChoices((current) => {
			if (current.some((choice) => choice.id === id))
				return current.filter((choice) => choice.id !== id);
	...
	const updateValues = (id, values) =>
		setChoices((current) =>
			current.map((choice) =>
				choice.id === id ? { ...choice, values } : choice,
			),
		);
	...
	const ready =
		...
		choices.every((choice) =>
			choice.id === "traits" || choice.id === "experiences"
				? choice.values.length === 2
				: choice.id === "domain_card" || choice.id === "multiclass"
	...
	const renderOption = (option, sourceTier = tier) => {
		...
		return (
			<div
				className={`${styles.advancementOption} ${choice ? styles.selectedAdvancement : ""}`}
				key={`${sourceTier}-${option.id}`}
			>
				<label>
					<input
						type="checkbox"
						...
						disabled={
							option.id === "domain_card" && availableCards.length === 0
						}
						onChange={() => toggleOption(option.id, sourceTier)}
					/>
					<span>
				...
				{choice?.id === "traits" && (
					<div className={styles.choiceDetails}>
						<span>Available traits</span>
						{availableTraits.map((trait) => (
							<label key={trait}>
								<input
									type="checkbox"
									checked={choice.values.includes(trait)}
									onChange={() => toggleValue("traits", trait)}
								/>
								{titleize(trait)}
							</label>
						))}
				...
				{choice?.id === "experiences" && (
					<div className={styles.choiceDetails}>
						<span>Experiences to improve</span>
						{(character.experiences || []).map((item, index) => (
							<label key={index}>
								<input
									type="checkbox"
									checked={choice.values.includes(index)}
									onChange={() => toggleValue("experiences", index)}
								/>
								{item.name || item}
							</label>
						))}
				...
				{choice?.id === "domain_card" && (
					<div className={styles.choiceDetails}>
						<span>Domain card (current level or lower)</span>
						<select
							value={choice.value ? `${choice.domain_id}:${choice.value}` : ""}
							onChange={(event) => {
						...
						>
							...
							{Object.entries(
								availableCards.reduce(
									(groups, card) => ({
										...groups,
										[card.domainId]: [...(groups[card.domainId] || []), card],
									}),
									{},
								),
							).map(([domainId, cards]) => (
								<optgroup label={cards[0].domain} key={domainId}>
									{cards.map((card) => (
										<option
											value={`${card.domainId}:${card.id}`}
											key={`${card.domainId}:${card.id}`}
										>
				...
				{choice?.id === "multiclass" && (
					<div className={styles.choiceDetails}>
						<span>Additional class</span>
						<select
							value={choice.value || ""}
							onChange={(event) =>
								setChoices((current) =>
						...
						>
							...
							{(book?.classes || [])
								.filter((item) => item.id !== character.class_id)
								.map((item) => (
	...
	return (
		<section className={styles.advancementPanel} id="character-advancement">
			<div className={styles.advancementHeader}>
				<div>
					<p className="eyebrow">
						LEVEL {character.level} · TIER {tier}
					</p>
					<h3>
						{nextLevel <= 10
							? `Advance to level ${nextLevel}`
							: "Character at maximum level"}
					</h3>
				</div>
			...
			{nextLevel <= 10 ? (
				<>
					<p className={styles.hint}>
						Tier {tier} lets you choose from this tier and every option from
						earlier tiers. One of your two choices must be an unowned domain
						card from your class.
					</p>
					{availableCards.length === 0 && (
						<p className={styles.error}>
							No valid unowned domain card is available for this level. Add an
							eligible card to the content book before advancing.
						</p>
					)}
					{milestoneLevels.has(nextLevel) && (
						<label className={styles.milestone}>
							<span>Additional Experience at +2</span>
							<input
								value={experience}
								onChange={(event) => setExperience(event.target.value)}
								placeholder="Name the new Experience"
							/>
						</label>
					)}
					...
					{tier >= 3 && (
						<div className={styles.previousUpgrades}>
							<button
								type="button"
								className={styles.textButton}
								onClick={() => setShowPreviousUpgrades((current) => !current)}
							>
								{showPreviousUpgrades
									? "Hide previous unmarked upgrades"
									: "View previous unmarked upgrades"}
							</button>
							{showPreviousUpgrades && (
								<>
									<p className={styles.hint}>
										Choose an upgrade from Tier {previousTier} that has not been
										marked there yet.
									</p>
									{previousOptions.length > 0 ? (
										<div className={styles.advancementOptions}>
											{previousOptions.map((option) =>
												renderOption(option, previousTier),
											)}
										</div>
									) : (
										<p className={styles.hint}>
											All previous-tier upgrades are already marked.
										</p>
									)}
								</>
							)}
					...
					<button
						...
						onClick={submit}
					>
						{saving ? "Saving level..." : `Save level ${nextLevel}`}
					</button>
				</>
			) : (
				<p className={styles.hint}>
					All ten levels have been recorded for this character.
				</p>
			)}
			<div className={styles.advancementHistory}>
				<h4>Advancement history</h4>
				{(character.advancements || []).length === 0 && (
					<p className={styles.hint}>No levels beyond level 1 yet.</p>
				)}
				{(character.advancements || []).map((entry) => (
					<div className={styles.historyEntry} key={entry.level}>
						<strong>Level {entry.level}</strong>
						<span>
							{entry.experience ? `New Experience: ${entry.experience}` : ""}
						</span>
						{(entry.choices || []).map((choice, index) => (
							<small key={`${entry.level}-${index}`}>
								{choiceSummary(choice, character, book)}
							</small>
						))}
					</div>
				))}
...
function CharacterEditor({
	...
	const input = (label, field, type = "input", maxLength) => (
	...
	return (
		<section className={styles.editor}>
			...
			<div className={styles.editorGrid}>
				...
				{input("Hair color", "hair_color")}
				{input("Skin color", "skin_color")}
				{input("Look description", "look_description", "textarea")}
			</div>
				...
				<div className={styles.editorGrid}>
					<label className={styles.editField}>
						...
						<input
							...
							onChange={(event) =>
								updateEquipmentField("primary", event.target.value)
							}
						/>
					</label>
					<label className={styles.editField}>
						...
						<input
							...
							onChange={(event) =>
								updateEquipmentField("secondary", event.target.value)
							}
						/>
					</label>
					<label className={styles.editField}>
						<span>Armor</span>
						<input
							value={form.equipment.armor || ""}
							onChange={(event) =>
								updateEquipmentField("armor", event.target.value)
							}
						/>
					</label>
				</div>
			...
			<div className={styles.editorSection}>
				<h4>Inventory</h4>
				{form.equipment.inventory.map((item, index) => (
					<div className={styles.editorRow} key={item.id || index}>
						<input
							value={item.name}
							onChange={(event) =>
								updateInventoryItem(index, "name", event.target.value)
							}
							placeholder="Item"
						/>
						<input
			...
			<div className={styles.editorSection}>
				<h4>Background</h4>
				<div className={styles.editorGrid}>
					{input("Birth city", "birth_city", "input", 160)}
					{input("Background story", "background_story", "textarea")}
					{input("Background notes", "background_notes", "textarea")}
					{input("Connections", "connections", "textarea")}
				</div>
				{form.family_members.map((member, index) => (
					<div className={styles.familyEditRow} key={member.id || index}>
						<select
							value={member.relation || "Other"}
							onChange={(event) =>
	...
	return (
		<div className={styles.weapon}>
			...
			<div className={styles.statLine}>
				<div>
					<span>Name</span>
					<strong>{weapon?.name || titleize(fallback) || "—"}</strong>
				</div>
				<div>
					<span>Trait &amp; range</span>
					<strong>
						{weapon
							? `${titleize(weapon.trait)} · ${titleize(weapon.range)}`
							: "—"}
					</strong>
				</div>
				<div>
