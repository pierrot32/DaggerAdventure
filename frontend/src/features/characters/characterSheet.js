...
export function parseThresholds(value) {
	const [major, severe] = String(value ?? "")
		.split("/")
		.map((part) => Number.parseInt(part, 10));
	return {
		major: Number.isFinite(major) ? major : 0,
		severe: Number.isFinite(severe) ? severe : 0,
	};
}
...
export function advancementEffects(advancements) {
	...
	(Array.isArray(advancements) ? advancements : []).forEach((entry) => {
		(Array.isArray(entry?.choices) ? entry.choices : []).forEach((choice) => {
			if (choice?.id === "traits")
				(choice.values || []).forEach((trait) => {
					if (trait in effects.traitBonuses) effects.traitBonuses[trait] += 1;
				});
			if (choice?.id === "experiences")
				(choice.values || []).forEach((index) => {
					effects.experienceBonuses[index] =
						(effects.experienceBonuses[index] || 0) + 1;
				});
...
export function deriveSheet(character, book) {
	...
	const subclassInfo =
		classInfo?.subclasses?.find((item) => item.id === character?.subclass_id) ||
		null;

	const equipment = character?.equipment || {};
	const primary = findEquipment(book, "primary_weapons", equipment.primary);
	const secondary = findEquipment(
		book,
		"secondary_weapons",
		equipment.secondary,
	);
	...
	return {
		...
		proficiency:
			1 +
			[2, 5, 8].filter((milestone) => level >= milestone).length +
...
export function normalizeStats(stats, derived) {
	...
	return {
		...source,
		hit_points: {
			current: clamp(source.hit_points?.current, derived.hitPointsMax),
			max: derived.hitPointsMax,
		},
		stress: {
			current: clamp(source.stress?.current, derived.stressMax),
			max: derived.stressMax,
		},
		hope: {
			current: clamp(source.hope?.current, derived.hopeMax),
			max: derived.hopeMax,
		},
		armor: {
			current: clamp(source.armor?.current, derived.armorScore),
			max: derived.armorScore,
		},
		gold: {
