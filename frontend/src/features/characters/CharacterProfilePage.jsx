...
export default function CharacterProfilePage() {
	...
	if (state.loading)
		return <p className="muted">Loading character profile...</p>;
	if (!character)
		return (
		...
		<section className={styles.page}>
			<header className={styles.header}>
				<div>
					...
					<p className="muted">
						The story, appearance, and relationships behind the sheet.
					</p>
				</div>
				{isOwner && (
					<Link
			...
			<div className={styles.hero}>
				<div className={styles.imageFrame}>
					{character.portrait_url ? (
						<img
							...
							alt={`${character.name}'s character portrait`}
						/>
					) : (
				...
				<div className={styles.identity}>
					...
					<h3>
						{titleize(character.class_id)} · {titleize(character.subclass_id)}
					</h3>
				...
				<div className={styles.mainColumn}>
					<Panel title="Description">
						<p className={styles.prose}>
							{character.description || "No description recorded."}
						</p>
					</Panel>
					<Panel title="Appearance">
						...
						<p className={styles.prose}>
							{character.look_description ||
								"No additional look details recorded."}
						</p>
					</Panel>
					<Panel title="Background">
						<p className={styles.prose}>
							{character.background_story || "No background story recorded."}
						</p>
						{character.background_notes && (
							<p className={styles.secondaryProse}>
								{character.background_notes}
							</p>
						)}
					</Panel>
				...
				<aside className={styles.sideColumn}>
					<Panel title="Connections">
						{connections.length > 0 ? (
							...
							<p className="muted">No connections recorded.</p>
						)}
					</Panel>
						...
						{family.length > 0 ? (
							<ul className={styles.familyList}>
								{family.map((member, index) => (
									<li key={member.id || `${member.relation}-${index}`}>
										<strong>
											{member.name || "Unnamed"} · {member.relation || "Other"}
										</strong>
										<span>{member.details || "No details recorded."}</span>
									</li>
								))}
							</ul>
						) : (
							<p className="muted">No family members recorded.</p>
						)}
					</Panel>
				</aside>
			</div>
