...
function Identity({ classes, ancestries, communities, frameContent, selectedClass, selectedSubclass, selectedAncestry, selectedFirstAncestry, selectedSecondAncestry, selectedCommunity, subclasses, form, setField, setClass, setAncestry, setFirstAncestry, setSubclass, locks, toggleLock, generate }) {
  ...
  return <div className={styles.formGrid}>
    ...
    <FieldLabel field="class_id" label="Class" info={<ModificationInfo label="Class campaign-frame guidance" entries={classGuidance} />} locked={locks.class_id} toggleLock={toggleLock} generate={generate}><select value={form.classId} onChange={(event) => setClass(event.target.value)}><option value="">Choose a class</option>{classes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></FieldLabel>
