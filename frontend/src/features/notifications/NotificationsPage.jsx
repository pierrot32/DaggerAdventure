...
export default function NotificationsPage() {
	const { notifications, loading, error, fetchNotifications, markRead } =
	...
	return (
		<section>
			...
			<h2>Notifications</h2>
			{(error || inviteError) && (
				<p className={styles.error}>{error || inviteError}</p>
			)}
			...
			{pendingInvites.length === 0 ? (
				...
				<div className={styles.list}>
					{pendingInvites.map((invite) => (
						<article
							className={`${styles.item} ${styles.invite}`}
							key={invite.id}
						>
							<div>
							...
							<div className={styles.inviteActions}>
								<Button type="button" onClick={() => respond(invite.id, true)}>
									Accept
								</Button>
								<Button
									type="button"
				...
				<div className={styles.list}>
					...
					{notifications.map((notification) => (
						...
						>
							...
							{notification.adventure_id && (
								<Link to={`/adventures/${notification.adventure_id}`}>
									View adventure
								</Link>
							)}
