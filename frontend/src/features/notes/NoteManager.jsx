...
const emptyDraft = (sectionId = "", position = 0) => ({
	id: null,
	title: "",
	body: "",
...
const notePosition = (notes, sectionId, selectedNoteId) => {
	...
	const selectedIndex = sectionNotes.findIndex(
...
export default function NoteManager({
	...
	const activeSectionId =
		selectedSectionId &&
		orderedSections.some((section) => section.id === selectedSectionId)
			? selectedSectionId
	...
	const activeNotes = useMemo(
		() => orderedNotes.filter((note) => note.section_id === activeSectionId),
		[activeSectionId, orderedNotes],
	);
	const selectedNote = orderedNotes.find(
		(note) => note.id === selectedNoteId && note.section_id === activeSectionId,
	);

	useEffect(() => {
		if (!activeSectionId) return;
		if (activeSectionId !== selectedSectionId)
	...
	useEffect(() => {
		if (
			!blockedSectionId ||
			orderedSections.some((section) => section.id === blockedSectionId)
		)
			return;
	...
	useEffect(() => {
		...
		setDraft(
			nextNote
				? { ...nextNote }
				: emptyDraft(activeSectionId, activeNotes.length),
		);
	}, [activeNotes, activeSectionId, creating, dirty, selectedNote]);

	const confirmDiscard = () =>
		!dirty || window.confirm("Discard unsaved note changes?");
	...
	const chooseNote = (note) => {
		...
		setExpandedSections((current) => ({ ...current, [note.section_id]: true }));
	...
	const startNewNote = () => {
		...
		setDraft(
			emptyDraft(
				activeSectionId,
				notePosition(orderedNotes, activeSectionId, selectedNoteId),
			),
		);
		setDirty(false);
	};
	...
	const save = async (event) => {
		...
		if (
			!targetSectionId ||
			targetSectionId === blockedSectionId ||
			!orderedSections.some((section) => section.id === targetSectionId)
		)
			return;
		const position = draft.id
			? draft.position
			: (draft.position ??
				notePosition(orderedNotes, targetSectionId, selectedNoteId));
	...
	const remove = async () => {
		...
		setSelectedNoteId(null);
		setCreating(false);
		setDraft(emptyDraft(activeSectionId, activeNotes.length));
		setDirty(false);
	};
	...
	const createSection = async (event) => {
		...
		if (created) {
			...
			setExpandedSections((current) => ({ ...current, [created.id]: true }));
			setSelectedNoteId(null);
			setCreating(true);
	...
	const startRename = (section) => {
		if (readOnly || saving || !confirmDiscard()) return;
		renameActionRef.current = "editing";
	...
	const deleteSection = async () => {
		...
		const draftTargetsDeletedSection =
			dirty && (draft.section_id || activeSectionId) === sectionId;
		if (
			draftTargetsDeletedSection &&
			!window.confirm(
				"Delete this section and keep the unsaved note draft in another section?",
			)
		)
			return;
		const deletion = await onDeleteSection(sectionId);
		if (!deletion || deletion.deleted !== true) return;
		const canonicalSections = Array.isArray(deletion.sections)
			? deletion.sections
			: null;
		const remainingSection = canonicalSections?.find(
	...
	const dropPosition = (note, sectionId, targetNoteId) => {
		const targetNotes = orderedNotes.filter(
			(item) => item.section_id === sectionId,
		);
		const sameSection = note.section_id === sectionId;
		const sourceIndex = sameSection
			? targetNotes.findIndex((item) => item.id === note.id)
			: -1;
		...
		const targetIndex = targetNotes.findIndex(
			(item) => item.id === targetNoteId,
		);
		if (targetIndex === -1) return null;
		return sameSection && sourceIndex < targetIndex
			? targetIndex - 1
			: targetIndex;
	};
	...
	const handleDrop = async (event, targetSectionId, targetNoteId = null) => {
		event.preventDefault();
		const note = orderedNotes.find(
			(item) => item.id === event.dataTransfer.getData("text/plain"),
		);
		if (note) await move(note, targetSectionId, targetNoteId);
	};
	const dropCandidates = (note, sectionId) =>
		orderedNotes.filter(
			(item) => item.section_id === sectionId && item.id !== note.id,
		);
	const candidateIndex = (candidates, targetNoteId) =>
		targetNoteId === null
			? candidates.length
			: candidates.findIndex((item) => item.id === targetNoteId);
	const startKeyboardDrag = (note) => {
		if (readOnly || !chooseNote(note)) return;
		const candidates = dropCandidates(note, note.section_id);
		const sourceIndex = orderedNotes
			.filter((item) => item.section_id === note.section_id)
			.findIndex((item) => item.id === note.id);
		setKeyboardDrag({
			noteId: note.id,
			targetSectionId: note.section_id,
			targetNoteId: candidates[sourceIndex]?.id || null,
		});
	};
	const moveKeyboardTarget = (note, direction) => {
		const currentSectionIndex = orderedSections.findIndex(
			(section) => section.id === keyboardDrag.targetSectionId,
		);
		const currentCandidates = dropCandidates(
			note,
			keyboardDrag.targetSectionId,
		);
		const currentIndex = candidateIndex(
			currentCandidates,
			keyboardDrag.targetNoteId,
		);
		if (direction === "up" || direction === "down") {
			...
			setKeyboardDrag((current) => ({
				...current,
				targetNoteId: currentCandidates[nextIndex]?.id || null,
			}));
			return;
		}
		const nextSectionIndex =
			currentSectionIndex + (direction === "left" ? -1 : 1);
		...
		setKeyboardDrag((current) => ({
			...
			targetNoteId: nextCandidates[nextIndex]?.id || null,
		}));
	};
	const handleNoteKeyDown = async (event, note) => {
		const isKeyboardDragging = keyboardDrag?.noteId === note.id;
		if (!isKeyboardDragging && event.key === " ") {
			event.preventDefault();
			startKeyboardDrag(note);
			return;
		}
		if (!isKeyboardDragging && event.key === "Enter") {
			event.preventDefault();
			chooseNote(note);
			return;
		}
	...
	const keyboardTargetSection =
		keyboardDrag &&
		orderedSections.find(
			(section) => section.id === keyboardDrag.targetSectionId,
		);
	const keyboardTargetNote =
		keyboardDrag &&
		orderedNotes.find((note) => note.id === keyboardDrag.targetNoteId);
	...
	return (
		<section className={styles.manager}>
			<div className={styles.heading}>
				...
				{!readOnly && (
					<Button type="button" variant="text" onClick={startNewNote}>
						New note
					</Button>
				)}
			</div>
			...
			{loading ? (
				...
				<div className={styles.layout}>
					<aside
						...
						aria-label={`${title} note navigator`}
					>
						{!readOnly && (
							<div className={styles.sectionCreation}>
								{addingSection ? (
									<form className={styles.inlineForm} onSubmit={createSection}>
										<label htmlFor={`${title}-new-section`}>New section</label>
										<div>
											<input
						...
						<div className={styles.sectionList}>
							{orderedSections.map((section) => {
								const sectionNotes = orderedNotes.filter(
									(note) => note.section_id === section.id,
								);
								const expanded = Boolean(expandedSections[section.id]);
								return (
									<div
										className={`${styles.sectionGroup} ${section.id === activeSectionId ? styles.activeGroup : ""}`}
										key={section.id}
									...
									>
										<div className={styles.sectionHeader}>
											...
											{!readOnly && section.id === activeSectionId && (
												<Button
													type="button"
													variant="text"
											...
											<div className={styles.noteLinks}>
												{sectionNotes.map((note) => (
													<button
														...
														key={note.id}
														draggable={!readOnly}
														className={`${note.id === selectedNoteId ? styles.selectedNoteLink : ""} ${keyboardDrag?.targetNoteId === note.id ? styles.keyboardDropTarget : ""}`}
														aria-describedby={
															!readOnly ? keyboardHelpId : undefined
														}
														aria-pressed={keyboardDrag?.noteId === note.id}
														onClick={() => chooseNote(note)}
														onKeyDown={(event) =>
															handleNoteKeyDown(event, note)
														}
														onDragStart={(event) =>
														...
														onDrop={(event) => {
															event.preventDefault();
															event.stopPropagation();
															handleDrop(event, section.id, note.id);
														}}
													>
					...
					<div
						...
						onDragOver={(event) => event.preventDefault()}
						onDrop={(event) => handleDrop(event, activeSectionId)}
					>
						{activeSection && (
							<div className={styles.contentHeading}>
								<div>
									<p className="eyebrow">{activeSection.name}</p>
									<h4>{selectedNote ? selectedNote.title : "New note"}</h4>
								</div>
						...
						{readOnly ? (
							<>
								{selectedNote ? (
									<article className={styles.readOnlyNote}>
								...
								<p className={styles.readOnly} role="status">
									Read-only view for the adventure GM.
								</p>
							</>
						) : (
							<form className={styles.editor} onSubmit={save}>
								...
								<div className={styles.actions}>
									<Button
										...
										disabled={
											saving ||
											blockedSectionId === (draft.section_id || activeSectionId)
										}
									>
