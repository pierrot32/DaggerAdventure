...
function recordEntries(value, fallbackGroup) {
  ...
  if (Array.isArray(value)) return value.flatMap((entry) => recordEntries(entry, group));
  if (!value || typeof value !== 'object') return [];
  if (value.id || value.name || value.category || value.description || value.text) return [{ ...value, group }];
  return Object.entries(value).flatMap(([key, entries]) => recordEntries(entries, canonicalGroup(key)));
}
...
function loadEquipment(book) {
  ...
  return sources.map((item, index) => {
    ...
    return { ...item, group, groupLabel: groupLabels[group] || item.category || group, key };
  }).filter((item) => {
...
function numericRange(value) {
  ...
  if (/^\d+(?:\.\d+)?$/.test(normalized)) return [Number(normalized), Number(normalized)];
  return null;
}
...
function rollRange(item) {
  ...
  for (const value of directValues) {
    ...
    if (value && typeof value === 'object') {
      const objectRange = numericRange(value.range ?? value.value ?? value.roll)
        || (Number.isFinite(Number(value.min)) && Number.isFinite(Number(value.max)) ? [Number(value.min), Number(value.max)] : null)
        || (Number.isFinite(Number(value.from)) && Number.isFinite(Number(value.to)) ? [Number(value.from), Number(value.to)] : null);
  ...
  if (item.table && typeof item.table === 'object' && !Array.isArray(item.table)) {
    ...
    if (ranges.length > 0) return [Math.min(...ranges.map((range) => range[0])), Math.max(...ranges.map((range) => range[1]))];
  }
  return null;
}
...
export default function EquipmentPage() {
  ...
  const groups = useMemo(() => [...new Set(equipment.map((item) => item.group))], [equipment]);
    ...
    <section className={styles.page}>
      <header className={styles.heading}>
        <div>
          ...
          <p className="muted">Browse armor, weapons, items, and consumables from the current content book.</p>
        </div>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.groupFilter} aria-label="Equipment type">
          ...
          <div className={styles.groupButtons}>
            ...
            {groups.map((id) => <button type="button" className={selectedGroup === id ? styles.selected : ''} aria-pressed={selectedGroup === id} onClick={() => setSelectedGroup(id)} key={id}>{groupLabels[id] || id}</button>)}
          </div>
        </div>
              ...
              {filteredEquipment.map((item) => {
                ...
                return (
                  <tr key={item.key}>
                    ...
                    <td data-label="Category">{displayValue(item.category || item.groupLabel)}</td>
                    <td data-label="Description" className={styles.feature}>{displayValue(item.description || item.text)}</td>
                    <td data-label="Source">{displayValue(item.source || item.source_url)}</td>
                    <td data-label="Tier">{displayValue(item.tier)}</td>
                    <td data-label="Details / roll">
                      {isArmor ? `Thresholds ${displayValue(item.thresholds)} · Armor ${displayValue(item.armor_score)}` : item.group === 'item' || item.group === 'consumable' ? displayValue(item.feature) : `${displayValue(item.trait)} · ${displayValue(item.range)} · ${displayValue(item.damage)} · ${displayValue(item.burden)}${item.feature ? ` · ${item.feature}` : ''}`}
                      {displayRollValue(item) && <span className={styles.rollValue}>Supplied roll: {displayRollValue(item)}</span>}
                      {rollRange(item) && canRandomRoll && <Button type="button" variant="text" onClick={() => rollItem(item)}>Roll{rolls[item.key] !== undefined ? `: ${rolls[item.key]}` : ''}</Button>}
                      {rollRange(item) && !canRandomRoll && <span className={styles.rollHint}>GM roll available</span>}
                    </td>
                  </tr>
                );
              })}
