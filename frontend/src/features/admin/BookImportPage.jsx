...
export default function BookImportPage() {
  ...
  const chooseFile = (candidate) => {
    ...
    if (!candidate.name.toLowerCase().endsWith('.json')) {
      setFile(null);
      setState({ busy: false, error: 'Choose a .json book file.', message: '' });
      return;
    }
    setFile(candidate);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setState({ busy: false, error: '', message: `${imported.title} ${imported.version} imported successfully.` });
    } catch (error) {
      setState({ busy: false, error: error instanceof SyntaxError ? 'The selected file is not valid JSON.' : error.message, message: '' });
    }
  };
    ...
    <section className={styles.page}>
      ...
      <form onSubmit={submit} className={styles.form}>
        <label
          ...
          onDrop={handleDrop}
        >
          <span className={styles.dropTitle}>{file ? file.name : 'Drop a .json book file here'}</span>
