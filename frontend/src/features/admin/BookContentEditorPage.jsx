	...
	return (
		<div className={styles.featureList}>
			<div className={styles.sectionHeading}>
				<h4>{title}</h4>
				<button type="button" className={styles.smallButton} onClick={onAdd}>
					Add feature
				</button>
			</div>
			{items.map((item, index) => (
				<div className={styles.featureRow} key={`${title}-${index}`}>
					<input
						aria-label={`${title} feature name ${index + 1}`}
						value={item.name || ""}
						placeholder="Feature name"
						onChange={(event) => onChange(index, "name", event.target.value)}
					/>
					<textarea
						aria-label={`${title} feature text ${index + 1}`}
						value={item.text || ""}
					...
					<TableEditor
						table={item.table}
						onChange={(table) => onChange(index, "table", table)}
						label={`${title} feature ${index + 1} table`}
					/>
				</div>
			))}
...
function SubclassEditor({ subclass, onChange, onRemove }) {
	...
	const updateFeature = (collection, index, field, value) =>
		update(
			collection,
			subclass[collection].map((item, itemIndex) =>
	...
	const renderFeatureList = (collection, title) => (
		<FeatureList
			...
			onRemove={(index) =>
				update(
					collection,
					subclass[collection].filter((_, itemIndex) => itemIndex !== index),
				)
			}
			onChange={(index, field, value) =>
				updateFeature(collection, index, field, value)
			}
		/>
	);
		...
		<div className={styles.subclassEditor}>
			...
			<div className={styles.formGrid}>
				...
				<label className={styles.wide}>
					...
					<textarea
						value={subclass.site_description || ""}
				...
				<label>
					Spellcast trait
					<input
						value={subclass.spellcast_trait || ""}
						onChange={(event) => update("spellcast_trait", event.target.value)}
					/>
				</label>
			</div>
...
function BeastFormEditor({ form, features, onChange, onRemove }) {
	...
	return (
		<div className={styles.subclassEditor}>
			...
			<div className={styles.formGrid}>
				...
				<label>
					...
					<input
						...
						value={form.tier}
						onChange={(event) =>
							update("tier", Number(event.target.value) || 1)
						}
					/>
				</label>
				<label>
					...
					<input
						type="number"
						value={form.evasion_bonus}
						onChange={(event) =>
							update("evasion_bonus", Number(event.target.value) || 0)
						}
					/>
				</label>
				<label>
					...
					<input
						...
						onChange={(event) => update("attack_range", event.target.value)}
						placeholder="Melee"
					/>
				</label>
				<label className={styles.wide}>
					...
					<textarea
						...
						placeholder="Dire Wolf, Velociraptor, Sabertooth Tiger"
					/>
				</label>
				<label>
					Attack trait
					<input
						value={form.attack_trait}
						onChange={(event) => update("attack_trait", event.target.value)}
				...
				<label>
					...
					<input
						type="number"
						value={form.attack_bonus}
						onChange={(event) =>
							update("attack_bonus", Number(event.target.value) || 0)
						}
					/>
				</label>
				<label>
					Attack damage
					<input
						value={form.attack_damage}
						onChange={(event) => update("attack_damage", event.target.value)}
						placeholder="d12+8 phy"
					/>
				</label>
				<label className={styles.wide}>
					Gain advantage on
					<input
						...
						placeholder="attack, sneak, sprint"
					/>
				</label>
				<label className={styles.wide}>
					Carrier
				...
				<fieldset className={styles.featureChoices}>
					...
					{features.map((item) => (
						<label className={styles.featureChoice} key={item.id}>
							<input
								type="checkbox"
								...
								onChange={(event) =>
									update(
										...
										event.target.checked
											? [...form.feature_ids, item.id]
											: form.feature_ids.filter((id) => id !== item.id),
									)
								}
							/>
							<span>{item.name || "Unnamed feature"}</span>
						</label>
					))}
				</fieldset>
			</div>
			{features.length === 0 && (
				<p className="muted">
					No shared beast features exist yet. Add one from the Beast features
					page.
				</p>
			)}
		</div>
...
function WeaponEditor({ item, onChange, onRemove }) {
	...
	return (
		<div className={styles.editor}>
			<div className={styles.editorHeading}>
				<div>
					<p className="eyebrow">{item.groupLabel}</p>
			...
			<div className={styles.formGrid}>
				...
				<label>
					...
					<input
						...
						value={item.tier}
						onChange={(event) =>
							onChange("tier", Number(event.target.value) || 1)
						}
					/>
				</label>
				{field("Trait", "trait")}
				...
				{item.group !== "armor" && (
					<label>
						<span>Magic weapon</span>
						<input
							type="checkbox"
							checked={Boolean(item.is_magic)}
							onChange={(event) => onChange("is_magic", event.target.checked)}
						/>
					</label>
				)}
...
function DomainCardEditor({ card, onChange, onRemove }) {
	...
	return (
		<div className={styles.editor}>
			<div className={styles.editorHeading}>
				<div>
					<p className="eyebrow">DOMAIN CARD</p>
			...
			<div className={styles.formGrid}>
				...
				<label>
					...
					<input
						type="number"
						min="0"
						value={card.recall_cost}
						onChange={(event) =>
							onChange("recall_cost", Number(event.target.value) || 0)
						}
					/>
				</label>
		...
		<div className={styles.editor}>
			<div className={styles.editorHeading}>
				<div>
					...
					<h3>{ancestry.name || "Unnamed ancestry"}</h3>
				</div>
				<button
					type="button"
					className={styles.removeButton}
			...
			<div className={styles.formGrid}>
				...
				<label className={styles.wide}>
					Selection rules
					<textarea
						value={ancestry.selection_rules}
						onChange={(event) => update("selection_rules", event.target.value)}
					/>
				</label>
			</div>
			<FeatureList
				title="Ancestry features"
				items={ancestry.features}
				onAdd={() => update("features", [...ancestry.features, feature()])}
				onRemove={(index) =>
					update(
						"features",
						ancestry.features.filter((_, itemIndex) => itemIndex !== index),
					)
				}
				onChange={(index, field, value) =>
					update(
						"features",
						ancestry.features.map((item, itemIndex) =>
							itemIndex === index ? { ...item, [field]: value } : item,
						),
					)
				}
		...
		<div className={styles.editor}>
			...
			<div className={styles.formGrid}>
				...
				<label className={styles.wide}>
					Adjectives
					<textarea
						value={community.adjectives.join("\n")}
						onChange={(event) =>
							update(
								"adjectives",
								event.target.value
			...
			<FeatureList
				...
				items={community.features}
				onAdd={() => update("features", [...community.features, feature()])}
				onRemove={(index) =>
					update(
						"features",
						community.features.filter((_, itemIndex) => itemIndex !== index),
					)
				}
				onChange={(index, field, value) =>
					update(
						"features",
						community.features.map((item, itemIndex) =>
							itemIndex === index ? { ...item, [field]: value } : item,
						),
					)
				}
			...
			<details className={styles.tierGroup} key={tier}>
				...
				<div className={styles.tierItems}>
					{tierItems.map((item) => (
						<button
							type="button"
							className={`${styles.classButton} ${item.key === selectedKey ? styles.selected : ""}`}
							key={item.key}
							onClick={() => onSelect(item)}
						>
							<strong>{item.name || `Unnamed ${itemLabel}`}</strong>
...
export default function BookContentEditorPage() {
	...
	useEffect(() => {
		const selected = flattenBeastForms(classes).find(
			(item) => item.key === selectedBeastFormKey,
		);
	...
	useEffect(() => {
		const selected = flattenWeapons(content?.equipment).find(
			(item) => item.key === selectedWeaponKey,
		);
	...
	useEffect(() => {
		const selected = (content?.ancestries || []).find(
			(item) => item.id === selectedAncestryId,
		);
		setAncestryForm(selected ? clone(selected) : null);
	}, [content, selectedAncestryId]);

	useEffect(() => {
		const selected = (content?.communities || []).find(
	...
	useEffect(() => {
		const selected = (content?.domains || []).find(
	...
	useEffect(() => {
		const selected = (content?.frames || []).find(
		...
		const draftOverride = selected
			? frameDraftOverrides.current.get(selected.id)
			: null;
		setFrameForm(
			draftOverride
				? clone(draftOverride)
				: selected
					? contentToForm(selected)
					: null,
		);
	}, [content, selectedFrameId]);

	useEffect(() => {
		const selected = flattenDomainCards(content?.domains).find(
			(item) => item.key === selectedDomainCardKey,
		);
	...
	const selectedSubclass = classForm?.subclasses.find(
		(item) => item.id === selectedSubclassId,
	);
	const beastFeatures = (content?.[BEAST_FEATURES_KEY] || [])
		.slice()
	...
	const domainCards = flattenDomainCards(domains).filter(
		(item) => item.domainId === selectedDomainId,
	);
	...
	const activeWeaponGroup = weaponGroups.find(
		(group) => group.id === editorType,
	);
	const isWeaponEditor = Boolean(activeWeaponGroup);
	const updateClass = (field, value) =>
		setClassForm((current) => ({ ...current, [field]: value }));
	const updateHope = (field, value) =>
		updateClass("hope_feature", { ...classForm.hope_feature, [field]: value });
	const updateSubclass = (subclass) => {
		updateClass(
			"subclasses",
			classForm.subclasses.map((item) =>
				item.id === selectedSubclassId ? subclass : item,
			),
		);
	...
	const removeClass = () => {
		if (
			!classForm ||
			!window.confirm(`Remove ${classForm.name || classForm.id}?`)
		)
			return;
	...
	const addSubclass = () => {
		...
		updateClass("subclasses", [
			...classForm.subclasses,
			normalizeSubclass({ id, name: "New subclass" }),
		]);
	...
	const addBeastForm = () => {
		const owner =
			classes.find((item) => item.id === beastFormOwnerId) ||
		...
		setClasses((current) =>
			current.map((item) =>
				item.id === owner.id
					? { ...item, beast_forms: [...item.beast_forms, nextForm] }
					: item,
			),
		);
	...
	const removeBeastForm = () => {
		if (
			!beastFormForm ||
			!window.confirm(`Remove ${beastFormForm.name || beastFormForm.id}?`)
		)
			return;
		const remaining =
			classes
				.find((item) => item.id === beastFormOwnerId)
				?.beast_forms.filter((form) => form.id !== beastFormOriginalId) || [];
		const nextForms = classes.map((item) =>
			item.id === beastFormOwnerId
				? {
						...item,
						beast_forms: item.beast_forms.filter(
							(form) => form.id !== beastFormOriginalId,
						),
					}
				: item,
		);
		...
		setSelectedBeastFormKey(
			next ? beastFormKey(beastFormOwnerId, next.id) : "",
		);
	};

	const addWeapon = (group) => {
		const id = `new-${group}-${(content.equipment[group] || []).length + 1}`;
		const nextWeapon = normalizeWeapon(
			{ id, name: "New item", tier: 1 },
			group,
		);
		setContent((current) => ({
			...current,
			equipment: {
				...current.equipment,
				[group]: [...(current.equipment[group] || []), nextWeapon],
			},
		}));
	...
	const removeWeapon = () => {
		if (
			!weaponForm ||
			!window.confirm(`Remove ${weaponForm.name || weaponForm.id}?`)
		)
			return;
		setContent((current) => ({
			...current,
			equipment: {
				...current.equipment,
				[weaponOriginalGroup]: current.equipment[weaponOriginalGroup].filter(
					(item) => item.id !== weaponOriginalId,
				),
			},
		}));
	...
	const removeSubclass = () => {
		if (
			!selectedSubclass ||
			!window.confirm(`Remove ${selectedSubclass.name || selectedSubclass.id}?`)
		)
			return;
		const remaining = classForm.subclasses.filter(
			(item) => item.id !== selectedSubclassId,
		);
	...
	const addAncestry = () => {
		...
		setContent((current) => ({
			...current,
			ancestries: [...(current.ancestries || []), newAncestry(id)],
		}));
	...
	const removeAncestry = () => {
		if (
			!ancestryForm ||
			!window.confirm(`Remove ${ancestryForm.name || ancestryForm.id}?`)
		)
			return;
		const remaining = ancestries.filter(
			(item) => item.id !== selectedAncestryId,
		);
		setContent((current) => ({
			...current,
			ancestries: current.ancestries.filter(
				(item) => item.id !== selectedAncestryId,
			),
		}));
	...
	const addCommunity = () => {
		...
		setContent((current) => ({
			...current,
			communities: [...(current.communities || []), newCommunity(id)],
		}));
	...
	const removeCommunity = () => {
		if (
			!communityForm ||
			!window.confirm(`Remove ${communityForm.name || communityForm.id}?`)
		)
			return;
	...
	const addDomain = () => {
		...
		setContent((current) => ({
			...current,
			domains: [...(current.domains || []), nextDomain],
		}));
	...
	const removeDomain = () => {
		if (
			!domainForm ||
			!window.confirm(`Remove ${domainForm.name || domainForm.id}?`)
		)
			return;
		const remaining = domains.filter((item) => item.id !== selectedDomainId);
		setContent((current) => ({
			...current,
			domains: current.domains.filter((item) => item.id !== selectedDomainId),
		}));
	...
	const updateDomainCard = (field, value) =>
		setDomainCardForm((current) => ({ ...current, [field]: value }));

	const removeDomainCard = () => {
		if (
			!domainCardForm ||
			!window.confirm(`Remove ${domainCardForm.name || domainCardForm.id}?`)
		)
			return;
		setContent((current) => ({
			...current,
			domains: current.domains.map((item) =>
				item.id === selectedDomainId
					? {
							...item,
							[`level_${domainCardOriginalLevel}_cards`]: (
								item[`level_${domainCardOriginalLevel}_cards`] || []
							).filter((card) => card.id !== domainCardOriginalId),
						}
					: item,
			),
		}));
	...
	const addFrame = () => {
		...
		setContent((current) => ({
			...current,
			frames: [...(current.frames || []), nextFrame],
		}));
	...
	const removeFrame = () => {
		if (
			!frameForm ||
			!window.confirm(`Remove ${frameForm.name || frameForm.id}?`)
		)
			return;
		const remaining = frames.filter((item) => item.id !== frameOriginalId);
		setContent((current) => ({
			...current,
			frames: current.frames.filter((item) => item.id !== frameOriginalId),
		}));
	...
	const save = async () => {
		...
		if (
			editorType === "classes" &&
			(!classForm?.id || !classForm.name.trim())
		) {
			setState((current) => ({
				...current,
				error: "A class needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (
			editorType === "beastforms" &&
			(!beastFormForm?.id || !beastFormForm.name.trim())
		) {
			setState((current) => ({
				...current,
				error: "A beast form needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (
			editorType === "ancestries" &&
			(!ancestryForm?.id || !ancestryForm.name.trim())
		) {
			setState((current) => ({
				...current,
				error: "An ancestry needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (
			editorType === "communities" &&
			(!communityForm?.id || !communityForm.name.trim())
		) {
			setState((current) => ({
				...current,
				error: "A community needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (
			editorType === "domains" &&
			(!domainForm?.id || !domainForm.name.trim())
		) {
			setState((current) => ({
				...current,
				error: "A domain needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (
			editorType === "domains" &&
			domainCardForm &&
			(!domainCardForm.id || !domainCardForm.name.trim())
		) {
			setState((current) => ({
				...current,
				error: "A domain card needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (isWeaponEditor && (!weaponForm?.id || !weaponForm.name.trim())) {
			setState((current) => ({
				...current,
				error: "A weapon or armor item needs an ID and name.",
				message: "",
			}));
			return;
		}
		if (
			...
			(!frameForm?.id ||
				!frameForm.name.trim() ||
				!frameForm.pitch.trim() ||
				!frameForm.overview.trim())
		) {
			setState((current) => ({
				...current,
				error: "A campaign frame needs an ID, name, pitch, and overview.",
				message: "",
			}));
			return;
		}
		const frameSaveRevision =
			editorType === "frames" ? frameFormRevision.current : null;
		const frameSaveSelectedId =
			editorType === "frames" ? frameOriginalId : null;
		const nextContent = clone(content);
		if (editorType === "classes") {
			nextContent.classes = classes.map((item) =>
				item.id === selectedClassId ? classForm : item,
			);
			...
			nextContent.classes = classes.map((item) =>
				item.id === beastFormOwnerId
					? {
							...item,
							beast_forms: item.beast_forms.map((form) =>
								form.id === beastFormOriginalId ? persistedBeastForm : form,
							),
						}
					: item,
			);
		} else if (editorType === "ancestries") {
			nextContent.ancestries = nextContent.ancestries.map((item) =>
				item.id === selectedAncestryId ? clone(ancestryForm) : item,
			);
		} else if (editorType === "communities") {
			nextContent.communities = nextContent.communities.map((item) =>
				item.id === selectedCommunityId ? clone(communityForm) : item,
			);
			...
			nextContent.equipment[weaponOriginalGroup] = nextContent.equipment[
				weaponOriginalGroup
			].map((item) => (item.id === weaponOriginalId ? persistedWeapon : item));
		} else if (editorType === "domains") {
			const persistedDomain = clone(domainForm);
			nextContent.domains = nextContent.domains.map((item) =>
				item.id === selectedDomainId ? persistedDomain : item,
			);
			if (domainCardForm) {
				const persistedDomainCard = clone(domainCardForm);
				const targetLevel =
					Number(persistedDomainCard.level) || domainCardOriginalLevel;
				...
				nextContent.domains = nextContent.domains.map((item) => {
					...
					nextDomain[originalKey] = (nextDomain[originalKey] || []).filter(
						(card) => card.id !== domainCardOriginalId,
					);
					nextDomain[targetKey] = [
						...(nextDomain[targetKey] || []),
						persistedDomainCard,
					];
					return nextDomain;
		...
		nextContent.frames = (nextContent.frames || []).map((item) => {
			...
			return draftOverride
				? { ...item, ...draftToContent(draftOverride, content) }
				: item;
		});
		nextContent.character_creation = {
			...(nextContent.character_creation || {}),
			connections_prompt: connections,
		};
		...
		try {
			...
			const hasNewerFrameEdits =
				editorType === "frames" &&
				frameFormRevision.current !== frameSaveRevision;
			if (editorType === "frames") {
				const latestFrameDraft =
					frameDraftOverrides.current.get(frameSaveSelectedId) || frameForm;
				const savedFrame =
					normalized.frames.find((item) => item.id === frameForm.id) ||
					normalized.frames.find((item) => item.id === frameSaveSelectedId);
				if (hasNewerFrameEdits) {
					if (savedFrame)
						frameDraftOverrides.current.set(
							savedFrame.id,
							clone(latestFrameDraft),
						);
				} else {
					frameDraftOverrides.current.delete(frameSaveSelectedId);
					frameDraftOverrides.current.delete(frameForm.id);
				}
				skipFrameRehydrate.current = {
					content: normalized,
					preserve: hasNewerFrameEdits,
				};
			} else {
				frameDraftOverrides.current.clear();
			}
			...
			if (editorType === "classes") {
				...
				setSelectedBeastFormKey(
					beastFormKey(beastFormOwnerId, beastFormForm.id),
				);
			} else if (editorType === "ancestries") {
				...
				setSelectedDomainCardKey(
					domainCardForm
						? domainCardKey(
								domainForm.id,
								Number(domainCardForm.level) || domainCardOriginalLevel,
								domainCardForm.id,
							)
						: "",
				);
			} else if (editorType === "frames") {
				if (!hasNewerFrameEdits && selectedFrameId === frameSaveSelectedId)
	...
	const download = async () => {
		try {
			...
			const blob = new Blob([JSON.stringify({ books: exported }, null, 2)], {
	...
	if (!selectedBook || !content) {
		if (state.error)
			return (
				<section className={styles.notice}>
					...
					<Button
						type="button"
						onClick={() => setBooksRequest((current) => current + 1)}
					>
						Retry loading books
					</Button>
				</section>
		...
		return (
			<section className={styles.notice}>
				...
				<p className="muted">Import a book before editing its content.</p>
			</section>
		);
	}
	...
	return (
		<section className={styles.page}>
			<header className={styles.header}>
				<div>
					<p className="eyebrow">ADMINISTRATION / CONTENT</p>
					<h2>Book content studio</h2>
					<p className="muted">
						Edit stored classes, equipment, domains, frames, and cards, then
						save the complete book.
					</p>
				</div>
				<Button type="button" variant="text" onClick={download}>
					Export all books
				</Button>
			</header>
			<fieldset className={styles.workspace} disabled={state.saving}>
				<div className={styles.toolbar}>
					<label>
						...
						<select
							value={bookId}
							onChange={(event) =>
								openBook(books.find((book) => book.id === event.target.value))
							}
						>
							{books.map((book) => (
				...
				<div className={styles.entityTabs}>
					<button
						type="button"
						className={editorType === "classes" ? styles.activeTab : ""}
						onClick={() => setEditorType("classes")}
					>
						Classes
					</button>
					<button
						type="button"
						className={editorType === "beastforms" ? styles.activeTab : ""}
						onClick={() => setEditorType("beastforms")}
					>
						Beast forms
					</button>
					<button
						type="button"
						className={editorType === "ancestries" ? styles.activeTab : ""}
						onClick={() => setEditorType("ancestries")}
					>
						Ancestries
					</button>
					<button
						type="button"
						className={editorType === "communities" ? styles.activeTab : ""}
						onClick={() => setEditorType("communities")}
					>
						Communities
					</button>
					<button
						type="button"
						className={editorType === "domains" ? styles.activeTab : ""}
						onClick={() => setEditorType("domains")}
					>
						Domains
					</button>
					<button
						type="button"
						className={editorType === "frames" ? styles.activeTab : ""}
						onClick={() => setEditorType("frames")}
					>
						Frames
					</button>
					{weaponGroups.map((group) => (
						<button
							type="button"
							className={editorType === group.id ? styles.activeTab : ""}
							onClick={() => openWeaponGroup(group)}
							key={group.id}
						>
							{group.label}
						</button>
					...
					<Link
						to="/admin/content/books/beast-features"
						className={styles.featureLink}
					>
						Manage shared beast features
					</Link>
				</div>
				<div className={styles.layout}>
					{editorType === "classes" && (
						<aside className={styles.sidebar}>
							...
							{classes.map((item) => (
								<button
									type="button"
									className={`${styles.classButton} ${item.id === selectedClassId ? styles.selected : ""}`}
									key={item.id}
									onClick={() => setSelectedClassId(item.id)}
								>
									<strong>{item.name || "Unnamed class"}</strong>
									<span>{item.id}</span>
								</button>
							))}
						</aside>
					...
					{editorType === "beastforms" && (
						<aside className={styles.sidebar}>
							...
							<label className={styles.sidebarField}>
								Class
								<select
									value={beastFormOwnerId || selectedClassId}
									onChange={(event) => {
										setBeastFormOwnerId(event.target.value);
							...
							<TierGroups
								items={beastForms}
								selectedKey={selectedBeastFormKey}
								onSelect={(item) => {
									setSelectedBeastFormKey(item.key);
									setSelectedClassId(item.classId);
									setBeastFormOwnerId(item.classId);
								}}
								emptyLabel="No beast forms added."
								itemLabel="beast form"
							/>
						</aside>
					)}
					{editorType === "ancestries" && (
						<aside className={styles.sidebar}>
							...
							{ancestries.map((item) => (
								<button
									type="button"
									className={`${styles.classButton} ${item.id === selectedAncestryId ? styles.selected : ""}`}
									key={item.id}
									onClick={() => setSelectedAncestryId(item.id)}
								>
									<strong>{item.name || "Unnamed ancestry"}</strong>
									<span>{item.id}</span>
								</button>
							))}
						</aside>
					...
					{editorType === "communities" && (
						<aside className={styles.sidebar}>
							...
							{communities.map((item) => (
								<button
									type="button"
									className={`${styles.classButton} ${item.id === selectedCommunityId ? styles.selected : ""}`}
									key={item.id}
									onClick={() => setSelectedCommunityId(item.id)}
								>
									<strong>{item.name || "Unnamed community"}</strong>
									<span>{item.id}</span>
								</button>
							))}
						</aside>
					...
					{isWeaponEditor && (
						<aside className={styles.sidebar}>
							<div className={styles.sectionHeading}>
								<h3>{activeWeaponGroup.label}</h3>
								<button
									type="button"
									className={styles.smallButton}
									onClick={() => addWeapon(activeWeaponGroup.id)}
								>
									Add{" "}
									{activeWeaponGroup.label
										.toLowerCase()
										.replace("weapons", "weapon")}
								</button>
							</div>
							<TierGroups
								items={activeWeaponGroup.items}
								selectedKey={selectedWeaponKey}
								onSelect={(item) => setSelectedWeaponKey(item.key)}
								emptyLabel={`No ${activeWeaponGroup.label.toLowerCase()} added.`}
								itemLabel="item"
							/>
						</aside>
					)}
					{editorType === "domains" && (
						<aside className={styles.sidebar}>
							...
							{domains.map((item) => (
								<button
									type="button"
									className={`${styles.classButton} ${item.id === selectedDomainId ? styles.selected : ""}`}
									key={item.id}
									onClick={() => {
								...
								>
									<strong>{item.name || "Unnamed domain"}</strong>
									<span>{item.id}</span>
								</button>
							))}
							{domainForm && (
								<>
									...
									<LevelGroups
										items={domainCards}
										selectedKey={selectedDomainCardKey}
										onSelect={(item) => setSelectedDomainCardKey(item.key)}
										emptyLabel="No cards at this level."
									/>
								</>
							)}
						</aside>
					...
					{editorType === "frames" && (
						<aside className={styles.sidebar}>
							...
							{frames.map((item) => (
								<button
									type="button"
									className={`${styles.classButton} ${item.id === selectedFrameId ? styles.selected : ""}`}
									key={item.id}
									onClick={() => {
								...
								>
									<strong>{item.name || "Unnamed frame"}</strong>
									<span>{item.id}</span>
								</button>
							))}
							{frames.length === 0 && (
								<p className="muted">No campaign frames added.</p>
							)}
							{frameForm && (
								<nav
									className={styles.frameSections}
								...
								>
									...
									{frameEditorSections.map((section) => (
										<button
											type="button"
											className={
												frameActiveSection === section.id
													? styles.activeSection
													: ""
											}
											key={section.id}
											onClick={() => setFrameActiveSection(section.id)}
											aria-current={
												frameActiveSection === section.id ? "step" : undefined
											}
										>
											{section.label}
										</button>
						...
						<div className={styles.editor}>
							<div className={styles.editorHeading}>
								<div>
									...
									<h3>{classForm.name || "Unnamed class"}</h3>
								</div>
								<button
									type="button"
									className={styles.removeButton}
							...
							<div className={styles.formGrid}>
								<label>
									...
									<input
										value={classForm.id}
										onChange={(event) => updateClass("id", event.target.value)}
									/>
								</label>
								<label>
									Class name
									<input
										...
										onChange={(event) =>
											updateClass("name", event.target.value)
										}
									/>
								</label>
								<label className={styles.wide}>
									Class description
									<textarea
										value={classForm.site_description || ""}
										onChange={(event) =>
											updateClass("site_description", event.target.value)
										}
									/>
								</label>
								<label>
								...
								<label>
									...
									<input
										type="number"
										value={classForm.evasion}
										onChange={(event) =>
											updateClass("evasion", Number(event.target.value))
										}
									/>
								</label>
								<label>
									...
									<input
										type="number"
										value={classForm.hit_points}
										onChange={(event) =>
											updateClass("hit_points", Number(event.target.value))
										}
									/>
								</label>
								<label className={styles.wide}>
									Class items
									<textarea
										value={classForm.class_items.join("\n")}
										onChange={(event) =>
											updateClass(
												"class_items",
							...
							<div className={styles.panel}>
								...
								<div className={styles.formGrid}>
									<label>
										Name
										<input
											value={classForm.hope_feature.name || ""}
											onChange={(event) =>
												updateHope("name", event.target.value)
											}
										/>
									</label>
									<label className={styles.wide}>
										Description
										<textarea
											value={classForm.hope_feature.text || ""}
											onChange={(event) =>
												updateHope("text", event.target.value)
											}
										/>
									</label>
								</div>
							...
							<FeatureList
								...
								items={classForm.class_features}
								onAdd={() =>
									updateClass("class_features", [
										...classForm.class_features,
										feature(),
									])
								}
								onRemove={(index) =>
									updateClass(
										"class_features",
										classForm.class_features.filter(
								...
								onChange={(index, field, value) =>
									updateClass(
										"class_features",
										classForm.class_features.map((item, itemIndex) =>
											itemIndex === index ? { ...item, [field]: value } : item,
										),
									)
								}
							...
							<label className={styles.wide}>
								...
								<textarea
									value={classForm.background_questions.join("\n")}
									onChange={(event) =>
										updateClass(
							...
							<div className={styles.panel}>
								...
								<div className={styles.subclassTabs}>
									{classForm.subclasses.map((item) => (
										<button
											type="button"
											className={
												item.id === selectedSubclassId ? styles.activeTab : ""
											}
											key={item.id}
											onClick={() => setSelectedSubclassId(item.id)}
										>
											{item.name || "Unnamed subclass"}
										</button>
									))}
								</div>
								{selectedSubclass ? (
									<SubclassEditor
										...
										onChange={updateSubclass}
										onRemove={removeSubclass}
									/>
								) : (
									<p className="muted">Add a subclass to edit its features.</p>
								)}
							</div>
							<label className={styles.wide}>
								Connections prompt
							...
							<Button type="button" disabled={state.saving} onClick={save}>
								{state.saving ? "Saving book..." : "Save book content"}
							</Button>
						</div>
					)}
						...
						<div className={styles.editor}>
							<div className={styles.formGrid}>
								<label>
									Class
									<select
										value={beastFormOwnerId}
										onChange={(event) => {
											const nextClass = classes.find(
												(item) => item.id === event.target.value,
											);
											if (!nextClass || nextClass.id === beastFormOwnerId)
												return;
											setClasses((current) =>
												current.map((item) =>
													item.id === beastFormOwnerId
											...
											setSelectedBeastFormKey(
												beastFormKey(nextClass.id, beastFormForm.id),
											);
										}}
									>
										{classes.map((item) => (
							...
							<BeastFormEditor
								form={beastFormForm}
								features={beastFeatures}
								onChange={updateBeastForm}
								onRemove={removeBeastForm}
							/>
							{(state.error || state.message) && (
								<p
									className={state.error ? styles.error : styles.message}
							...
							<Button type="button" disabled={state.saving} onClick={save}>
								{state.saving ? "Saving book..." : "Save beast form"}
							</Button>
						</div>
					)}
					{editorType === "ancestries" && ancestryForm && (
						<AncestryEditor
							ancestry={ancestryForm}
							onChange={setAncestryForm}
							onRemove={removeAncestry}
						/>
					)}
					{editorType === "communities" && communityForm && (
						<CommunityEditor
						...
						<div className={styles.editor}>
							<div className={styles.editorHeading}>
								<div>
									...
									<h3>{domainForm.name || "Unnamed domain"}</h3>
								</div>
								<button
									type="button"
									className={styles.removeButton}
							...
							{domainCardForm ? (
								<DomainCardEditor
									card={domainCardForm}
									onChange={updateDomainCard}
									onRemove={removeDomainCard}
								/>
							) : (
								<p className="muted">
									Select a level in the sidebar to edit a domain card, or add a
									new card.
								</p>
							)}
							{(state.error || state.message) && (
						...
						<div className={styles.editor}>
							<div className={styles.editorHeading}>
								<div>
									...
									<h3>{frameForm.name || "Unnamed frame"}</h3>
								</div>
								<button
									type="button"
									className={styles.removeButton}
							...
							<Button type="button" disabled={state.saving} onClick={save}>
								{state.saving ? "Saving book..." : "Save frame"}
							</Button>
						</div>
					)}
				</div>
