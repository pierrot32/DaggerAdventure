...
export default function AdminAiLogsPage() {
	...
	return (
		<section>
			...
			<p className="muted">
				Review the compact requests and answers sent through the character
				generator.
			</p>
			{state.error && (
			...
			{state.loading ? (
				<p className="muted">Loading generation history...</p>
			) : (
				<div className={styles.list}>
					{logs.length === 0 && (
						<p className="muted">No generation requests yet.</p>
					)}
					{logs.map((log) => (
						<details key={log.id} className={styles.entry}>
							<summary>
								<span>
									<strong>{log.generation_type.replace("_", " ")}</strong>
