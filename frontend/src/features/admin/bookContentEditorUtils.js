...
export const DOMAIN_LEVELS = Array.from(
	{ length: 10 },
	(_, index) => index + 1,
);
...
export const newDomain = (id = "new-domain") => ({
	...
	classes: [],
	...Object.fromEntries(
		DOMAIN_LEVELS.map((level) => [`level_${level}_cards`, []]),
	),
});
...
export function normalizeSubclass(item) {
	return {
		...
		foundation: Array.isArray(item.foundation)
			? item.foundation.map((entry) => normalizeFeature(entry))
			: [],
		specialization: Array.isArray(item.specialization)
			? item.specialization.map((entry) => normalizeFeature(entry))
			: [],
		mastery: Array.isArray(item.mastery)
			? item.mastery.map((entry) => normalizeFeature(entry))
			: [],
	};
}
...
export function normalizeClass(item) {
	return {
		...
		class_features: Array.isArray(item.class_features)
			? item.class_features.map((entry) => normalizeFeature(entry))
			: [],
		beast_forms: Array.isArray(item.beast_forms)
			? item.beast_forms.map(normalizeBeastForm)
			: [],
		background_questions: Array.isArray(item.background_questions)
			? item.background_questions
			: [],
		subclasses: Array.isArray(item.subclasses)
			? item.subclasses.map(normalizeSubclass)
			: [],
...
export function normalizeDomain(item) {
	...
	DOMAIN_LEVELS.forEach((level) => {
		...
		normalized[key] = Array.isArray(item[key])
			? item[key].map(normalizeDomainCard)
			: [];
	});
	return normalized;
}
...
export function normalizeAncestry(item) {
	return {
		...
		features: Array.isArray(item.features)
			? item.features.map((entry) => normalizeFeature(entry))
			: [],
	};
}
	...
	const normalized = {
		...
		features: (Array.isArray(item.features)
			...
			: item.feature
				? [item.feature]
				: []
		).map((entry) => normalizeFeature(entry)),
	};
	...
	const classes = content.classes.map((classItem) => ({
		...
		beast_forms: classItem.beast_forms.map((form) => {
			...
			form.features.forEach((item, index) => {
				const baseId =
					item.id ||
					`${form.id}-${slugify(item.name || `feature-${index + 1}`)}`;
				let id = baseId;
				let suffix = 2;
				while (
					knownIds.has(id) &&
...
export function normalizeBookContent(value) {
	...
	content.classes = Array.isArray(content.classes)
		? content.classes.map(normalizeClass)
		: [];
	content.ancestries = Array.isArray(content.ancestries)
		? content.ancestries.map(normalizeAncestry)
		: [];
	content.communities = Array.isArray(content.communities)
		? content.communities.map(normalizeCommunity)
		: [];
	content.domains = Array.isArray(content.domains)
		? content.domains.map(normalizeDomain)
		: [];
	...
	["items", "consumables"].forEach((key) => {
		if (Array.isArray(content[key]))
			content[key] = content[key].map((item) =>
				normalizeLoot(item, key === "items" ? "item" : "consumable"),
			);
	});
	["item", "consumable"].forEach((key) => {
		if (Array.isArray(content.equipment[key]))
			content.equipment[key] = content.equipment[key].map((item) =>
				normalizeLoot(item, key),
			);
	});
...
export function flattenBeastForms(classes) {
	return classes
		.flatMap((classItem) =>
			classItem.beast_forms.map((form) => ({
				...form,
...
export function flattenWeapons(equipment) {
	return WEAPON_GROUPS.flatMap(({ id, label }) =>
		(equipment?.[id] || []).map((item) => ({
			...item,
			group: id,
			groupLabel: label,
			key: `${id}::${item.id}`,
...
export function flattenDomainCards(domains) {
	return (domains || [])
		.flatMap((domainItem) =>
			DOMAIN_LEVELS.flatMap((level) =>
				(domainItem[`level_${level}_cards`] || []).map((card) => ({
					...card,
					level,
