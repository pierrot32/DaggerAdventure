...
export default function BeastFeatureEditorPage() {
	...
	const removeFeature = () => {
		if (
			!featureForm ||
			!window.confirm(`Remove ${featureForm.name || featureForm.id}?`)
		)
			return;
	...
	const save = async () => {
		if (!content || !featureForm?.id || !featureForm.name.trim()) {
			setState((current) => ({
				...current,
				error: "A beast feature needs an ID and name.",
				message: "",
			}));
			return;
		}
		const nextContent = clone(content);
		nextContent[BEAST_FEATURES_KEY] = features.map((item) =>
			item.id === featureOriginalId ? featureForm : item,
		);
		const requestGeneration = bookRequestRef.current;
		const requestBookId = bookId;
		const isCurrentRequest = () =>
			bookRequestRef.current === requestGeneration && bookId === requestBookId;
	...
	if (state.error && (!selectedBook || !content))
		return (
	...
	if (!selectedBook || !content)
		return (
			<section className={styles.notice}>
				...
				<p className="muted">Import a book before editing beast features.</p>
			</section>
		);
		...
		<section className={styles.page}>
			<header className={styles.header}>
				<div>
					...
					<h2>Beast feature library</h2>
					<p className="muted">
						Create reusable features once, then assign them to any number of
						beast forms.
					</p>
				</div>
			...
			<div className={styles.toolbar}>
				...
				<div className={styles.links}>
					<Link to="/admin/content/books/edit">Book content studio</Link>
			...
			<div className={styles.layout}>
				<aside className={styles.sidebar}>
					...
					{features.map((item) => (
						<button
							type="button"
							className={`${styles.featureButton} ${item.id === selectedFeatureId ? styles.selected : ""}`}
							key={item.id}
							onClick={() => setSelectedFeatureId(item.id)}
						>
							<strong>{item.name || "Unnamed feature"}</strong>
							<span>{item.id}</span>
						</button>
					))}
					{features.length === 0 && (
						<p className="muted">No shared features added.</p>
					)}
				</aside>
				{featureForm && (
					<div className={styles.editor}>
						<div className={styles.editorHeading}>
							<div>
								<p className="eyebrow">SHARED BEAST FEATURE</p>
								<h3>{featureForm.name || "Unnamed feature"}</h3>
							</div>
							<button
								type="button"
						...
						<div className={styles.formGrid}>
							<label>
								...
								<input
									...
									onChange={(event) => updateFeature("id", event.target.value)}
								/>
							</label>
							<label>
								Feature name
						...
						<p className={styles.hint}>
							Beast forms can select this feature from their shared feature
							list. Editing it updates every form that uses it.
						</p>
						...
						<Button type="button" disabled={state.saving} onClick={save}>
							{state.saving ? "Saving feature..." : "Save beast feature"}
						</Button>
					</div>
				)}
			</div>
