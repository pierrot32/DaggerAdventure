...
export const createSoundSource = (payload) => request('/api/sound-sources', {
  method: 'POST',
  body: JSON.stringify(payload),
});
export const updateSoundSource = (sourceId, payload) => request(`/api/sound-sources/${sourceId}`, {
  method: 'PUT',
  body: JSON.stringify(payload),
});
export const deleteSoundSource = (sourceId) => request(`/api/sound-sources/${sourceId}`, { method: 'DELETE' });

export const listSoundLibrary = () => request('/api/sound-library');
export const uploadLibraryTrack = (formData) => request('/api/sound-library', {
  method: 'POST',
  body: formData,
});
export const deleteLibraryTrack = (trackId) => request(`/api/sound-library/${trackId}`, { method: 'DELETE' });
export const attachLibraryTrack = (boardId, trackId) => request(`/api/soundboards/${boardId}/library/${trackId}`, { method: 'POST' });
export const detachLibraryTrack = (boardId, trackId) => request(`/api/soundboards/${boardId}/library/${trackId}`, { method: 'DELETE' });
