...
export default function AdventureListPage() {
	...
	return (
		<section>
			...
			{loading ? (
				<p className="muted">Loading adventures...</p>
			) : (
				<div className={styles.grid}>
					{adventures.length === 0 && (
						<p className="muted">No adventures are available yet.</p>
					)}
					{adventures.map((adventure) => (
						<article className={styles.card} key={adventure.id}>
							<Link
							...
							{adventure.creator_id === user?.id && (
								<div className={styles.cardActions}>
									<span className={styles.ownerLabel}>Your game</span>
								</div>
							)}
						</article>
					))}
