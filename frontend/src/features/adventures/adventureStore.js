...
export const useAdventureStore = create((set) => ({
	...
	fetchAdventures: async () => {
		...
		try {
			set({ adventures: await adventureApi.listAdventures(), loading: false });
		} catch (error) {
	...
	fetchAdventure: async (id) => {
		...
		try {
			...
			if (requestGeneration === adventureFetchGeneration)
				set({ current: adventure, loading: false });
		} catch (error) {
			if (requestGeneration === adventureFetchGeneration)
				set({ error: error.message, loading: false });
		}
	...
	fetchPendingInvites: async () => {
		try {
			set({ pendingInvites: await adventureApi.listMyInvites() });
		} catch (error) {
