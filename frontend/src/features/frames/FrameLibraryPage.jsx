  ...
  return (
    <section className={styles.page}>
      <div className={styles.heading}>
        ...
        <Button type="button" onClick={() => navigate('/frames/new')}>New frame</Button>
      </div>
      ...
      {state.loading ? <p className="muted">Loading frame library...</p> : <div className={styles.layout}>
        <div className={styles.list}>
          ...
          {frames.map((frame) => <button type="button" className={styles.card} onClick={() => navigate(`/frames/${frame.id}/edit`)} key={frame.id}><strong>{frame.name}</strong><span>{frame.description || 'No description yet.'}</span><small>Complexity {frame.complexity_rating}/5</small></button>)}
        </div>
      </div>}
    </section>
  );
