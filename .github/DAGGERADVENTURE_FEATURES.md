...
- **Behavior:** Admins import, inspect, edit, and export JSON content books. The frame editor inside the book studio reuses the shared focused-section navigation and form: only the selected frame section's fields are rendered, while the frame heading, remove control, status, and save control remain visible. Book feature entries can have optional structured tables; the table editor lets admins choose initial row and column counts, then add or remove rows and columns. Table normalization and object spreading retain arbitrary table metadata while editing. The character builder reads the character-creation book and uses its classes, subclasses, ancestries, communities, equipment, and related options.
...
- **Validation:** Frontend `npm run lint`, `npm run build`, and `git diff --check` passed for the completed change. No dedicated frontend unit or browser coverage verifies focused frame navigation, book-feature tables, or arbitrary table metadata round trips.
...
- **Validation:** Frontend `npm run lint`, `npm run build`, and `git diff --check`, plus backend `cargo fmt -- --check` and `cargo test --lib`, passed for the completed change. Browser coverage and frontend unit-test coverage remain gaps for adventure-first context, the player-facing overlays, and edit-mode adventure-list failure/current-link behavior.
...
- **Behavior:** Adventure makers can browse the library, open a frame from the list, create through `/frames/new`, and edit through `/frames/:frameId/edit`. The editor keeps the draft in memory while its section selector changes the focused form section. Frame detail fields are shown only in the details section, while the frame title remains visible in the editor top bar. The admin book frame editor uses the same section model and shared form, hiding non-selected fields while keeping the frame title, remove control, status, and save control visible. Each section has a compact two-line GM note with vertical resize; GM notes remain separate from player-facing content. Tone, themes, touchstones, session-zero questions, player principles, GM principles, and distinctions use repeatable one-input fields. Campaign mechanics entries support optional structured tables with configurable initial rows and columns and later row/column changes; shared table normalization preserves arbitrary metadata keys. [`frameDraft.js`](../frontend/src/features/frames/frameDraft.js) accepts prior string/object forms, preserves object metadata such as tables during normalization, and serializes them back through the existing frame content shape. Selected community, ancestry, and class targets derive feature titles from their content-book names (joined with ` / `); unselected or unresolved targets retain the entered title or a kind-specific fallback while IDs are regenerated from frame, kind, and target names. Create-adventure keeps blank, built-in, and library frame sources and previews the selected frame's player-facing content.
- **Invariants:** The editor continues to use the existing library CRUD client calls and payload shape. `GET/POST/PUT/DELETE /api/frames/library...` remains authenticated and requires `adventure_maker` access or higher; repository list, update, delete, and library-source lookup are owner-scoped by `owner_id`. Preserve frame source type, source ID, content, selections, and arbitrary table metadata. Adventure-frame mutations remain creator-owned, while attached-frame reads and filtered character context retain their existing membership/access rules. No backend, database schema, or API ownership change is part of this reconciliation.
- **Validation:** Frontend `npm run lint`, `npm run build`, and `git diff --check`, plus backend `cargo fmt -- --check` and `cargo test --lib`, passed for the completed change. Browser coverage and frontend unit-test coverage remain gaps, including admin and dedicated-editor focused-section navigation, compact GM-note behavior, table row/column editing and metadata preservation, repeatable-field serialization, and save/delete flows.
...
- The campaign-frame editor and admin book frame editor have no browser-level coverage; route navigation, in-memory section switching, title/save/remove persistence, target-title derivation, GM-note controls, table editing/metadata preservation, and save/delete flows still need manual browser verification.
