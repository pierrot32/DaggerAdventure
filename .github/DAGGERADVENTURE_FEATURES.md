...
- **Behavior:** A player first chooses an accessible adventure or the standalone option in the builder. A direct `?adventure=<id>` selection remains usable even while the adventure list is loading or when that ID is not returned by the list; list and frame-load failures are surfaced without destroying that selection, and stale frame responses cannot replace newer route state. The selected adventure ID is sent on character creation and AI generation; standalone creation omits it. The builder's frame banner deliberately displays only the selected frame title and pitch. When selected, the attached filtered frame context is used for character guidance and choice-specific class, ancestry, and community hints. Modification matching unwraps nested content responses, reads current array and object-map entries, accepts plural/singular keys plus `target_ids`, legacy target arrays, `target_id`, `target`, or `targets`, and compares normalized target IDs, names, and titles. Direct target matches are preferred, then general name/title matches and broad entries, then any available entries from a populated frame kind; results are de-duplicated. Class/subclass overlays use the selected class guidance, and mixed-ancestry overlays evaluate both selected lineages. Class, subclass, ancestry, and community controls expose player-facing campaign modifications in dismissible information overlays, while selected content-book features and details are shown in the identity step. Character edit mode includes an adventure-linking dropdown; if the adventure list fails to load, the error is visible and the current linked adventure remains selectable/preserved. The character model includes identity, appearance, class/subclass, ancestry/community, traits, experiences, background, family, connections, equipment, domain cards, level, advancements, and an optional portrait.
...
- The character builder's adventure-first selection, standalone path, direct query compatibility, title/pitch-only banner, filtered frame-context display, direct/general/broad/available guidance fallback, current array/object-map and legacy target matching, mixed-ancestry and subclass overlays, and class/ancestry/community guidance have no browser-level or frontend unit-test coverage. Character edit-mode adventure-list failure and current-link preservation likewise lack focused browser or unit coverage.
