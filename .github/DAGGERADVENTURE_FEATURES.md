...
- **Behavior:** Admins or explicitly granted accounts can generate playground text. Character generation accepts selected unlocked fields, preserves locked values, validates choice IDs against allowed options, can expand supported long text fields, and can use the selected adventure's attached frame context. The character builder presents only the frame title and pitch to the player, while `generate_character` still passes the existing selection-filtered context, including overview when enabled, to the AI prompt. `gm_messages` and nested `gm_message` values are recursively removed before that prompt is sent. Portrait generation uses the stored character data and saves the resulting portrait URL. Requests and responses are logged for admin review.
- **Invariants:** `require_ai_generation` is mandatory. Keep API keys backend-only. Treat model output as untrusted proposal data, filter it to requested fields, validate choices, and preserve locked fields. Frame context is loaded through `frame_repo::find_for_user` and filtered by the existing frame route, preserving adventure membership/access and ownership rules. No backend endpoint, persistence schema, or API ownership change is part of this reconciliation.
- **Validation:** Frontend `npm run lint`, `npm run build`, and `git diff --check`, plus backend `cargo fmt -- --check` and `cargo test --lib`, passed for the completed change. Browser coverage and frontend unit-test coverage remain gaps for the context display and generation flow.
...
- The character builder's adventure-first selection, standalone path, direct query compatibility, title/pitch-only banner, filtered frame-context display, and class/subclass/ancestry/community overlays have no browser-level or frontend unit-test coverage. Character edit-mode adventure-list failure and current-link preservation likewise lack focused browser or unit coverage.
