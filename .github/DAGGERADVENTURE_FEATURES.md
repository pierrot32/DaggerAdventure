...
- **Behavior:** A player first chooses an accessible adventure or the standalone option in the builder. A direct `?adventure=<id>` selection remains usable even while the adventure list is loading or when that ID is not returned by the list. The selected adventure ID is sent on character creation and AI generation; standalone creation omits it. When selected, the attached filtered frame context is used for character guidance and choice-specific class, ancestry, and community hints, and the builder displays the frame pitch and overview. The character model includes identity, appearance, class/subclass, ancestry/community, traits, experiences, background, family, connections, equipment, domain cards, level, advancements, and an optional portrait.
- **Invariants:** Preserve locked or user-authored choices during generation and editing. Check character ownership before mutation. Keep character content compatible with the current content-book IDs. Adventure selection and frame context use the existing APIs and server-side membership/ownership rules; this change adds no backend or schema contract.
- **Validation:** Frontend `npm run lint`, `npm run build`, and `git diff --check`, plus backend `cargo fmt -- --check` and `cargo test --lib`, passed for the completed change. Browser coverage and frontend unit-test coverage remain gaps.
...
- **Behavior:** Adventure makers can browse the library, open a frame from the list, create through `/frames/new`, and edit through `/frames/:frameId/edit`. The editor keeps the draft in memory while its section selector changes the focused form section. Frame detail fields are shown only in the details section, while the frame title remains visible in the editor top bar. Each section has a compact two-line GM note with vertical resize; GM notes remain separate from player-facing content. Tone, themes, touchstones, session-zero questions, player principles, GM principles, and distinctions use repeatable one-input fields. [`frameDraft.js`](../frontend/src/features/frames/frameDraft.js) accepts their prior string/object forms and serializes them back through the existing frame content shape. Selected community, ancestry, and class targets derive feature titles from their content-book names (joined with ` / `); unselected or unresolved targets retain the entered title or a kind-specific fallback while IDs are regenerated from frame, kind, and target names. Create-adventure keeps blank, built-in, and library frame sources and previews the selected frame's player-facing content.
- **Invariants:** The editor continues to use the existing library CRUD client calls and payload shape. `GET/POST/PUT/DELETE /api/frames/library...` remains authenticated and requires `adventure_maker` access or higher; repository list, update, delete, and library-source lookup are owner-scoped by `owner_id`. Preserve frame source type, source ID, content, and selections. Adventure-frame mutations remain creator-owned, while attached-frame reads and filtered character context retain their existing membership/access rules. No backend or database schema change is part of this reconciliation.
- **Validation:** Frontend `npm run lint`, `npm run build`, and `git diff --check`, plus backend `cargo fmt -- --check` and `cargo test --lib`, passed for the completed change. Browser coverage and frontend unit-test coverage remain gaps, including focused-section navigation, compact GM-note behavior, repeatable-field serialization, and save/delete flows.
...
- **Behavior:** Admins or explicitly granted accounts can generate playground text. Character generation accepts selected unlocked fields, preserves locked values, validates choice IDs against allowed options, can expand supported long text fields, and can use the selected adventure's attached frame context. The existing filtered context path includes selected player-facing frame fields such as pitch and overview, while `gm_messages` and nested `gm_message` values remain excluded before they reach the AI prompt. Portrait generation uses the stored character data and saves the resulting portrait URL. Requests and responses are logged for admin review.
...
- The character builder's adventure-first selection, standalone path, direct query compatibility, and filtered frame-context display have no browser-level or frontend unit-test coverage.
